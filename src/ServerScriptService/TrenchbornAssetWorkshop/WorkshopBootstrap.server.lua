...
aegisDressing.Apply(aegisModel)
aegisModel:PivotTo(aegisModel:GetPivot() + Vector3.new(-55, 0, 85))
fleetRig.Apply(aegisModel, {AnchorRoot = true})
aegisGameplay.Attach(aegisModel, aegisGameplayConfig)
aegisReactions.Attach(aegisModel)
aegisSoundController.Attach(aegisModel)
fleetRigTestHarness.Attach(aegisModel)
workshop:SetAttribute("CurrentAsset", aegisSpecification.AssetName)
workshop:SetAttribute("CurrentPhase", 6)
workshop:SetAttribute("QualityStatus", "Phase6_GameplayReview")
