...
local specification = require(packageFolder:WaitForChild("LargeCityMeridianPharmaSpecification"))
local goldenMaster = require(packageFolder:WaitForChild("LargeCityMeridianPharmaGoldenMaster"))
local dressing = require(packageFolder:WaitForChild("LargeCityMeridianPharmaDressing"))

local Installer = {}

local MODEL_NAME = specification.AssetId
local GOLDEN_MASTER_NAME = "LargeCity_MeridianPharma_L3_GoldenMaster"
...
local function validateDestructionGroups(model)
	...
	assert(groups and groups:IsA("Folder"), "Large City Meridian Pharma installer requires a DestructionGroups folder")

	for _, groupName in ipairs(specification.PlannedDestructionGroups) do
		local group = groups:FindFirstChild(groupName)
		assert(group and group:IsA("Folder"), "Large City Meridian Pharma is missing destruction group: " .. groupName)
	end
...
local function placeModel(model, options)
	if options.CFrame then
		assert(typeof(options.CFrame) == "CFrame", "Large City Meridian Pharma options.CFrame must be a CFrame")
		model:PivotTo(options.CFrame)
		return
	end

	local groundCFrame = options.GroundCFrame or CFrame.new()
	assert(typeof(groundCFrame) == "CFrame", "Large City Meridian Pharma options.GroundCFrame must be a CFrame")
...
local function applyIntegrationMetadata(model)
	...
	model:SetAttribute("PackageId", "LargeCityMeridianPharmaPackage")
...
function Installer.Validate(model)
	assert(model and model:IsA("Model"), "LargeCityMeridianPharmaInstaller.Validate requires a Model")
	validateDestructionGroups(model)
	assert(model:GetAttribute("MaxHealth") == MAX_HEALTH, "Large City Meridian Pharma MaxHealth metadata is invalid")
	assert(model:GetAttribute("EnergyType") == ENERGY_TYPE, "Large City Meridian Pharma EnergyType metadata is invalid")
	assert(CollectionService:HasTag(model, BUILDING_TAG), "Large City Meridian Pharma is missing tag: " .. BUILDING_TAG)
	return true
end

function Installer.Install(parent, options)
	assert(typeof(parent) == "Instance", "LargeCityMeridianPharmaInstaller.Install requires a parent Instance")
	options = options or {}
	assert(typeof(options) == "table", "LargeCityMeridianPharmaInstaller.Install options must be a table")
...
function Installer.Uninstall(parent)
	assert(typeof(parent) == "Instance", "LargeCityMeridianPharmaInstaller.Uninstall requires a parent Instance")
