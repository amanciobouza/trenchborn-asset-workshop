-- Phase 7: approved Stage 2 Storm Hunter asset. Call from the server, after avatar appearance loads.
...
local Installer={Version="1.0.3",ApprovedRevision="803c8ae4789f87b1a20ba6fa5433ecfda5e49675"}
...
local function build(parent,ground,options)
 return StageTwo.Build(parent,ground,options)
end
function Installer.Install(character,options)
 ...
 local ok,err=pcall(function()
  ...
  if options.InstallInput and player then
   ...
   assert(not gui:FindFirstChild("StormHunterInput"),"Storm Hunter input already installed")
   inputGui=Instance.new("ScreenGui");inputGui.Name="StormHunterInput";inputGui.ResetOnSpawn=true
   local input=script.Parent:WaitForChild("KaijuStageTwoInput"):Clone()
   input.Parent=inputGui;inputGui.Parent=gui
  end
  if player then
   local gui=assert(player:FindFirstChildOfClass("PlayerGui"),"PlayerGui not ready")
   assert(not gui:FindFirstChild("StormHunterMotor"),"Jump motor already installed")
   motorGui=Instance.new("ScreenGui");motorGui.Name="StormHunterMotor";motorGui.ResetOnSpawn=true
