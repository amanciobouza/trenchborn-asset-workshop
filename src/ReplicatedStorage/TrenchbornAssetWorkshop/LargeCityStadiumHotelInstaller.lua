...
local dressing = require(packageFolder:WaitForChild("LargeCityStadiumHotelDressing"))

local Installer = {}

local MODEL_NAME = specification.AssetId
local GOLDEN_MASTER_NAME = "LargeCity_StadiumHotel_L3_GoldenMaster"
...
local function validateDestructionGroups(model)
	...
	assert(groups and groups:IsA("Folder"), "Large City Stadium Hotel installer requires a DestructionGroups folder")

	for _, groupName in ipairs(specification.PlannedDestructionGroups) do
		local group = groups:FindFirstChild(groupName)
		assert(group and group:IsA("Folder"), "Large City Stadium Hotel is missing destruction group: " .. groupName)
	end
...
local function placeModel(model, options)
	if options.CFrame then
		assert(typeof(options.CFrame) == "CFrame", "Large City Stadium Hotel options.CFrame must be a CFrame")
		model:PivotTo(options.CFrame)
		return
	end

	local groundCFrame = options.GroundCFrame or CFrame.new()
	assert(typeof(groundCFrame) == "CFrame", "Large City Stadium Hotel options.GroundCFrame must be a CFrame")
...
local function applyIntegrationMetadata(model)
	...
	model:SetAttribute("PackageId", "LargeCityStadiumHotelPackage")
...
function Installer.Validate(model)
	assert(model and model:IsA("Model"), "LargeCityStadiumHotelInstaller.Validate requires a Model")
	validateDestructionGroups(model)
	assert(model:GetAttribute("MaxHealth") == MAX_HEALTH, "Large City Stadium Hotel MaxHealth metadata is invalid")
	assert(model:GetAttribute("EnergyType") == ENERGY_TYPE, "Large City Stadium Hotel EnergyType metadata is invalid")
	assert(CollectionService:HasTag(model, BUILDING_TAG), "Large City Stadium Hotel is missing tag: " .. BUILDING_TAG)
	return true
end

function Installer.Install(parent, options)
	assert(typeof(parent) == "Instance", "LargeCityStadiumHotelInstaller.Install requires a parent Instance")
	options = options or {}
	assert(typeof(options) == "table", "LargeCityStadiumHotelInstaller.Install options must be a table")
...
function Installer.Uninstall(parent)
	assert(typeof(parent) == "Instance", "LargeCityStadiumHotelInstaller.Uninstall requires a parent Instance")
