...
local GOLDEN_MASTER_NAME = "LargeCity_CentralHospital_L3_GoldenMaster"
local PACKAGE_VERSION = 1
local MAX_HEALTH = specification.GameplayMetadata.TargetMaxHealth
local ENERGY_TYPE = specification.GameplayMetadata.EnergyType
local BUILDING_TAG = specification.GameplayMetadata.InstallerTag

local function validateDestructionGroups(model)
	local groups = model:FindFirstChild("DestructionGroups")
	assert(groups and groups:IsA("Folder"), "Central Hospital installer requires a DestructionGroups folder")

	for _, groupName in ipairs(specification.PlannedDestructionGroups) do
		local group = groups:FindFirstChild(groupName)
		assert(group and group:IsA("Folder"), "Central Hospital is missing destruction group: " .. groupName)
	end
...
local function placeModel(model, options)
	if options.CFrame then
		assert(typeof(options.CFrame) == "CFrame", "Central Hospital options.CFrame must be a CFrame")
		model:PivotTo(options.CFrame)
		return
	end

	local groundCFrame = options.GroundCFrame or CFrame.new()
	assert(typeof(groundCFrame) == "CFrame", "Central Hospital options.GroundCFrame must be a CFrame")
...
local function applyIntegrationMetadata(model)
	...
	model:SetAttribute("PackageId", "LargeCityCentralHospitalPackage")
...
function Installer.Validate(model)
	assert(model and model:IsA("Model"), "LargeCityCentralHospitalInstaller.Validate requires a Model")
	validateDestructionGroups(model)
	assert(model:GetAttribute("MaxHealth") == MAX_HEALTH, "Central Hospital MaxHealth metadata is invalid")
	assert(model:GetAttribute("EnergyType") == ENERGY_TYPE, "Central Hospital EnergyType metadata is invalid")
	assert(CollectionService:HasTag(model, BUILDING_TAG), "Central Hospital is missing tag: " .. BUILDING_TAG)
	return true
end

function Installer.Install(parent, options)
	assert(typeof(parent) == "Instance", "LargeCityCentralHospitalInstaller.Install requires a parent Instance")
	options = options or {}
	assert(typeof(options) == "table", "LargeCityCentralHospitalInstaller.Install options must be a table")
...
function Installer.Uninstall(parent)
	assert(typeof(parent) == "Instance", "LargeCityCentralHospitalInstaller.Uninstall requires a parent Instance")
