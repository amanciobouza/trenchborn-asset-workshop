...
function Rig.Attach(model, movementRoot, humanoid, combat)
	...
	local specialLock
	local function holdSpecial()
		if not specialLock then return end
		humanoid.WalkSpeed=0;humanoid.AutoRotate=false;humanoid.Jump=false
		humanoid:Move(Vector3.zero,false)
		movementRoot.Anchored=true;movementRoot.CFrame=specialLock.Frame
	...
	local function beginSpecial(kind)
		assert(not specialLock,"Special attack already active")
		specialLock={Frame=movementRoot.CFrame,Anchored=movementRoot.Anchored,
			Speed=humanoid.WalkSpeed,Rotate=humanoid.AutoRotate,
			JumpEnabled=humanoid:GetStateEnabled(Enum.HumanoidStateType.Jumping)}
		humanoid:SetStateEnabled(Enum.HumanoidStateType.Jumping,false)
		hitReaction=nil;strikeResistance=nil
		wasWalking=false;stopAge=nil;runBlend=0;walkFeet={};lastWalkUpper={}
		model:SetAttribute("Running",false);model:SetAttribute("ReactionState","Idle")
		model:SetAttribute("SpecialAttackLocked",kind)
		holdSpecial()
	end
	local function releaseSpecial()
		if not specialLock then return end
		local saved=specialLock;specialLock=nil
		if movementRoot.Parent then
			movementRoot.AssemblyLinearVelocity=Vector3.zero
			movementRoot.AssemblyAngularVelocity=Vector3.zero
			movementRoot.Anchored=saved.Anchored
		end
		...
		model:SetAttribute("SpecialAttackLocked",nil)
	end
	...
	heartbeat = RunService.Heartbeat:Connect(function(dt)
		...
		if focus and (humanoid.Health<=0 or os.clock()-focus.Started>=4.85) then endFocus() end
		local focusTime=focus and os.clock()-focus.Started
		if area and (humanoid.Health<=0 or os.clock()-area.Started>=AREA_TIMING.Finish) then endArea() end
		...
		for _, event in ipairs(combo:DrainEvents()) do
			if event.Kind=="Whoosh" then
				feedback("Whoosh",event.Index==1 and bones.LeftHand or bones.RightHand,true)
