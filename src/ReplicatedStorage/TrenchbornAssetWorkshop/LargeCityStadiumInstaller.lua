...
local GOLDEN_MASTER_NAME = "LargeCity_Stadium_L3_GoldenMaster"
...
local function validateDestructionGroups(model)
	...
	assert(groups and groups:IsA("Folder"), "Large City Stadium installer requires a DestructionGroups folder")

	for _, groupName in ipairs(specification.PlannedDestructionGroups) do
		local group = groups:FindFirstChild(groupName)
		assert(group and group:IsA("Folder"), "Large City Stadium is missing destruction group: " .. groupName)
	end
...
local function placeModel(model, options)
	if options.CFrame then
		assert(typeof(options.CFrame) == "CFrame", "Large City Stadium options.CFrame must be a CFrame")
		model:PivotTo(options.CFrame)
		return
	end

	local groundCFrame = options.GroundCFrame or CFrame.new()
	assert(typeof(groundCFrame) == "CFrame", "Large City Stadium options.GroundCFrame must be a CFrame")
...
local function applyIntegrationMetadata(model)
	...
	model:SetAttribute("BuildingType", "Stadium")
	...
	model:SetAttribute("PackageId", "LargeCityStadiumPackage")
...
local function configurePong(model, options)
	local enabled = options.EnablePong == true
	model:SetAttribute("PongEnabled", enabled)
	model:SetAttribute("PongIncluded", true)

	if enabled then
		pong.Attach(model)
	else
		model:SetAttribute("PongRuntimeAttached", false)
	end
end

function Installer.Validate(model)
	assert(model and model:IsA("Model"), "LargeCityStadiumInstaller.Validate requires a Model")
	validateDestructionGroups(model)
	assert(model:GetAttribute("MaxHealth") == MAX_HEALTH, "Large City Stadium MaxHealth metadata is invalid")
	assert(model:GetAttribute("EnergyType") == ENERGY_TYPE, "Large City Stadium EnergyType metadata is invalid")
	assert(CollectionService:HasTag(model, BUILDING_TAG), "Large City Stadium is missing tag: " .. BUILDING_TAG)
	return true
end

function Installer.Install(parent, options)
	assert(typeof(parent) == "Instance", "LargeCityStadiumInstaller.Install requires a parent Instance")
	options = options or {}
	assert(typeof(options) == "table", "LargeCityStadiumInstaller.Install options must be a table")
...
function Installer.Uninstall(parent)
	assert(typeof(parent) == "Instance", "LargeCityStadiumInstaller.Uninstall requires a parent Instance")
