...
local specification = require(packageFolder:WaitForChild("LargeCitySummitTowerSpecification"))
local goldenMaster = require(packageFolder:WaitForChild("LargeCitySummitTowerGoldenMaster"))
local dressing = require(packageFolder:WaitForChild("LargeCitySummitTowerDressing"))

local Installer = {}

local MODEL_NAME = specification.AssetId
local GOLDEN_MASTER_NAME = "LargeCity_SummitTower_L3_GoldenMaster"
...
local function validateDestructionGroups(model)
	...
	assert(groups and groups:IsA("Folder"), "Large City Summit Tower installer requires a DestructionGroups folder")

	for _, groupName in ipairs(specification.PlannedDestructionGroups) do
		local group = groups:FindFirstChild(groupName)
		assert(group and group:IsA("Folder"), "Large City Summit Tower is missing destruction group: " .. groupName)
	end
...
local function placeModel(model, options)
	if options.CFrame then
		assert(typeof(options.CFrame) == "CFrame", "Large City Summit Tower options.CFrame must be a CFrame")
		model:PivotTo(options.CFrame)
		return
	end

	local groundCFrame = options.GroundCFrame or CFrame.new()
	assert(typeof(groundCFrame) == "CFrame", "Large City Summit Tower options.GroundCFrame must be a CFrame")
...
local function applyIntegrationMetadata(model)
	...
	model:SetAttribute("PackageId", "LargeCitySummitTowerPackage")
...
function Installer.Validate(model)
	assert(model and model:IsA("Model"), "LargeCitySummitTowerInstaller.Validate requires a Model")
	validateDestructionGroups(model)
	assert(model:GetAttribute("MaxHealth") == MAX_HEALTH, "Large City Summit Tower MaxHealth metadata is invalid")
	assert(model:GetAttribute("EnergyType") == ENERGY_TYPE, "Large City Summit Tower EnergyType metadata is invalid")
	assert(CollectionService:HasTag(model, BUILDING_TAG), "Large City Summit Tower is missing tag: " .. BUILDING_TAG)
	return true
end

function Installer.Install(parent, options)
	assert(typeof(parent) == "Instance", "LargeCitySummitTowerInstaller.Install requires a parent Instance")
	options = options or {}
	assert(typeof(options) == "table", "LargeCitySummitTowerInstaller.Install options must be a table")
...
function Installer.Uninstall(parent)
	assert(typeof(parent) == "Instance", "LargeCitySummitTowerInstaller.Uninstall requires a parent Instance")
