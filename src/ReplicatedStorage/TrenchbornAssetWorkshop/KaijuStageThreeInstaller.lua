-- Phase 7: approved Stage 3 Rift Stalker asset. Call from the server, after avatar appearance loads.
...
local Installer={Version="1.1.5",ApprovedRevision="8a88c8ab7fe9e36ce6b7c6ea3f9c911942513456"}
...
local function build(parent,ground,options)
 return StageThree.Build(parent,ground,options)
end
function Installer.Install(character,options)
 ...
 local ok,err=pcall(function()
  ...
  if options.InstallInput and player then
   ...
   assert(not gui:FindFirstChild("RiftStalkerInput"),"Rift Stalker input already installed")
   inputGui=Instance.new("ScreenGui");inputGui.Name="RiftStalkerInput";inputGui.ResetOnSpawn=true
   local input=script.Parent:WaitForChild("KaijuStageThreeInput"):Clone()
   input.Parent=inputGui;inputGui.Parent=gui
  end
  if player then
   local gui=assert(player:FindFirstChildOfClass("PlayerGui"),"PlayerGui not ready")
   assert(not gui:FindFirstChild("RiftStalkerMotor"),"Jump motor already installed")
   motorGui=Instance.new("ScreenGui");motorGui.Name="RiftStalkerMotor";motorGui.ResetOnSpawn=true
