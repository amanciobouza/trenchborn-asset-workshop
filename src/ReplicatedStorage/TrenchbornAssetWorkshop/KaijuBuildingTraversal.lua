-- Shared Stage 1–4 traversal. Registered buildings only; damage stays in
...
function Traversal.Attach(model,root,humanoid,rootHeight,collider,combat,player)
 ...
 local folder=Instance.new("Folder");folder.Name="KaijuBuildingTraversal";folder.Parent=character
 ...
 local report=remote.OnServerEvent:Connect(function(sender,side,point)
  if not player or sender~=player or player.Character~=character or not feet[side] or not finite(point) then return end
 ...
 model:SetAttribute("TraversalRevision","S1_S4_FreeFacingTranslationCollision_05")
