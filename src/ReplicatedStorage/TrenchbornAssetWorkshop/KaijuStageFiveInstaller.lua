-- Phase 7 final Stage 5 installer; user approved geometry, dressing and gameplay.
...
local StageFive=require(script.Parent:WaitForChild("KaijuStageFiveGoldenMaster"))
local Rig=require(script.Parent:WaitForChild("KaijuStageOneRig"))
local Installer={Version="1.0.0",ApprovedRevision="53316a87cc4a205e6f1c134afc90b158fb5c3c7d"}
...
local function stamp(model)
 ...
 model:SetAttribute("RuntimeReview","ApprovedByUser_Workshop")
 model:SetAttribute("Purpose","Stage 5 final asset package")
 model:SetAttribute("FinalInstallerVersion",Installer.Version)
 model:SetAttribute("IntegrationReview","PendingInTargetProject")
 model:SetAttribute("TestOnly",false)
 model:SetAttribute("WorkshopOnly",false)
 model:SetAttribute("SailPresentation","TwoAnchorClientGeometry")
end

local function build(parent,ground,options)
 local model=StageFive.Build(parent,ground,options)
 local renderer=Instance.new("ObjectValue");renderer.Name="KaijuSailRenderer"
 renderer.Value=script.Parent:WaitForChild("KaijuEnergySailPresentation");renderer.Parent=model
 return model
end
function Installer.Install(character,options)
 ...
 assert(not installations[character] and not character:FindFirstChild(NAME)
  ...
  and not character:FindFirstChild("Stage_4_Geometry_Review")
 ...
 local ok,err=pcall(function()
  ...
  if options.InstallInput and player then
   ...
   assert(not gui:FindFirstChild("StageFiveInput"),"Stage 5 input already installed")
   inputGui=Instance.new("ScreenGui");inputGui.Name="StageFiveInput";inputGui.ResetOnSpawn=true
   local input=script.Parent:WaitForChild("KaijuStageFiveInput"):Clone()
   input.Parent=inputGui;inputGui.Parent=gui
  end
  if player then
   local gui=assert(player:FindFirstChildOfClass("PlayerGui"),"PlayerGui not ready")
   assert(not gui:FindFirstChild("StageFiveMotor"),"Jump motor already installed")
   motorGui=Instance.new("ScreenGui");motorGui.Name="StageFiveMotor";motorGui.ResetOnSpawn=true
