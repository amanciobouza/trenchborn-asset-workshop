...
function Library.BuildShoulderMissileSalvo()
	...
	keyframe(sequence, 2.53, brace(0.1))
	-- The second shoulder group starts after the visible pod switch.
	keyframe(sequence, 2.64, brace(1))
	keyframe(sequence, 2.73, brace(0.2))
	keyframe(sequence, 2.84, brace(0.85))
	keyframe(sequence, 2.93, brace(0.2))
	keyframe(sequence, 3.04, brace(0.7))
	keyframe(sequence, 3.18, brace(0.1))
	keyframe(sequence, 3.35, brace(0))
	keyframe(sequence, 3.62, {})

	sequence:SetAttribute("GuardianAnimation", "ShoulderMissileSalvo")
	sequence:SetAttribute("DurationSeconds", 3.62)
