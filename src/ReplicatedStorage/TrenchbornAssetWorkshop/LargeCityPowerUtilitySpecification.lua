...
local Specification = {
	...
	TechnicalBreakdown = {
		...
		SwitchgearHall = {
			Footprint = Vector2.new(68, 44),
			Center = Vector3.new(-38, 16, -19),
			Height = 32,
		...
		ConverterHall = {
			Footprint = Vector2.new(62, 42),
			Center = Vector3.new(34, 20, -17),
			Height = 40,
		...
		TransformerCourt = {
			Face = "+Z",
			CourtCenter = Vector3.new(0, 0, 34),
			TransformerCount = 4,
			TransformerBodySize = Vector3.new(16, 18, 12),
			TransformerSpacing = 32,
			CoolingFinDepth = 2.0,
			BushingHeight = 8,
			MaintenanceClearance = 5,
			Rule = "Four large transformers form their own parallel row at Z=34 with wide walkable gaps between units; no loading doors or unrelated clutter may obstruct the row.",
		},
		ReactivePowerYard = {
			PadSize = Vector2.new(148, 48),
			CapacitorBankCount = 4,
		...
		BusbarGantries = {
			...
			BusbarDiameter = 1.5,
			SwitchBayCount = 12,
			ParallelSwitchRows = 2,
			ServiceCorridorCount = 4,
			InsulatorHeight = 3.4,
			Rule = "Four full-width portal rows and two complete six-bay switch rows create a deep, walkable substation field. Busbars remain rigid conductors carried on insulators, never refinery-like pipework.",
		},
		RooftopSystems = {
			BaseY = 32,
			MaximumY = 56,
	...
	Phase4Status = {
		...
		ReviewFocus = {
			...
			"four capacitor banks and three shunt reactors form a complete rear reactive-power row",
			...
			"four full-width portal rows establish a large switchyard silhouette",
			"two complete parallel six-bay switch rows are separated by a walkable service aisle",
			"four clear service corridors allow the player to move between the halls, switch rows, transformer row and reactive-power row",
