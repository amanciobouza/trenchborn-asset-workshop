...
$env:CHATGPT_WORKSPACE_AGENT_ID = "agtch_your_published_agent_id"
$env:CHATGPT_WORKSPACE_AGENT_TOKEN = "your_workspace_agent_access_token"
...
The bridge then triggers the published ChatGPT Workspace Agent, using the same
conversation key on every iteration. Git credentials and a Workspace Agent
access token must be available locally. No OpenAI API key is required: visual
review invokes `codex exec` using the cached ChatGPT login.
...
change improves the result. The triggered ChatGPT agent reads `reviews/latest/`
and corrects the model in Git. Final approval always remains with the user.
