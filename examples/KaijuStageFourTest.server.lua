-- Phase 6 Studio playtest. Disable the workshop preview before enabling.
local RunService=game:GetService("RunService")
if not RunService:IsStudio() then
 warn("[Stage 4 test] Practice bootstrap runs only in Studio")
 return
end
local Players=game:GetService("Players")
local package=game:GetService("ReplicatedStorage"):WaitForChild("TrenchbornKaijuStageFourTest")
local Installer=require(package:WaitForChild("KaijuStageFourTestInstaller"))
...
local function equip(player,character)
 ...
 if not ok then pending[character]=nil;warn("[Stage 4 test] "..tostring(err)) end
end
...
print("[Stage 4 test] Ready: 3 houses + 2 towers. F combo, Space jump, E focus, R area; Shift run.")
