...
local package=game:GetService("ReplicatedStorage"):WaitForChild("TrenchbornKaijuStageTwo")
local Installer=require(package:WaitForChild("KaijuStageTwoInstaller"))
...
local function equip(player,character)
 ...
 if not ok then pending[character]=nil;warn("[Storm Hunter] "..tostring(err)) end
end
