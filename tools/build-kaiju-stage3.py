"""Build the portable Rift Stalker model and verify its embedded sources."""
import hashlib
import json
import re
...
FILES=['KaijuBuildingTraversal.lua','KaijuPresentation.lua','KaijuPresentationClient.client.lua','KaijuPresentationBootstrap.lua','KaijuSkeleton.lua','KaijuStageThreeInstaller.lua','KaijuStageThreeGoldenMaster.lua','KaijuStageTwoGoldenMaster.lua','KaijuEvolutionBlockout.lua',
       'KaijuStageOneRig.lua','KaijuStageOneCombo.lua','KaijuStageOneJump.lua',
       'KaijuStageThreeInput.client.lua','KaijuStageOneJumpMotor.client.lua']
def build():
    ...
    ET.SubElement(props,'string',name='Name').text='TrenchbornKaijuStageThree'
    installer=(SOURCE/'KaijuStageThreeInstaller.lua').read_text(encoding='utf-8')
    version=re.search(r'Version="([^"]+)"',installer).group(1)
    revision=re.search(r'ApprovedRevision="([^"]+)"',installer).group(1)
    manifest={'package':'TrenchbornKaijuStageThree','version':version,
              'approvedRevision':revision,'files':{}}
    ...
    target=out/'KaijuStageThree.rbxmx';target.write_bytes(data)
    manifest['artifactSha256']=hashlib.sha256(data).hexdigest()
    (out/'KaijuStageThree.manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
