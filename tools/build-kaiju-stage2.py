"""Build the portable Storm Hunter model and verify its embedded sources."""
import hashlib
import json
import re
...
FILES=['KaijuBuildingTraversal.lua','KaijuPresentation.lua','KaijuPresentationClient.client.lua','KaijuPresentationBootstrap.lua','KaijuSkeleton.lua','KaijuStageTwoInstaller.lua','KaijuStageTwoGoldenMaster.lua','KaijuEvolutionBlockout.lua',
...
def build():
    ...
    target=out/'KaijuStageTwo.rbxmx';target.write_bytes(data)
    manifest['artifactSha256']=hashlib.sha256(data).hexdigest()
    (out/'KaijuStageTwo.manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
