...
Area accepts ground normals with Y >= 0.4 and builds the brace frame using that normal and the projected facing direction. Ground must be within 40 scaled studs of the root, matching the existing ray endpoint; the workshop damage adapter uses the same bound. The 150-stud death ray is intentionally unchanged because it locates the corpse landing surface, not an attack target. Rejections return false plus a reason and set AreaRejectReason. Ground failures also set AreaPhase to "No ground". A successful request clears the rejection reason. Game adapters should commit energy/cooldown costs only after RequestArea returns true; this change cannot refund or change costs already applied by external game code.
