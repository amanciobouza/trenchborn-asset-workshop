# Storm Hunter — portable package 1.1.5
...
2. Replace the complete `ReplicatedStorage.TrenchbornKaijuStageTwo` folder. Do not mix old rig, jump motor and new presentation modules.
...
12 embedded scripts include geometry, installer, shared server controller, immutable skeleton builder, client presentation, shared observer/bootstrap, combo/jump timing, optional stage input and owner jump motor. Names containing StageOne are shared dependencies; the equipped figure remains Storm Hunter.

Run `python tools/build-kaiju-stage2.py` or `rojo build kaiju-stage2.project.json -o KaijuStageTwo.rbxm`. The Python builder checks exact embedded source round-trips; the manifest records source and artifact hashes. `python tools/test-kaiju-replication.py` exercises mocked runtime contracts across all five stage numbers.
